...
def dora_marker_to_dora(dora_marker):
    hai = get_hai34(dora_marker)
    if hai < 27 and hai % 9 == 8:
        return hai - 8
    elif hai == 30:
        return 27
    elif hai == 33:
...
class Player_State:
    def __init__(self, score, jikaze, bakaze, tehai, nokori_array, honba, kyotaku, dora_marker):
        self.score = score
        self.jikaze = jikaze
        self.bakaze = bakaze
        self.tehai = tehai
    ...
    def to_numpy_look_ahead_feature(self):
        args = ['./a.out']
        te = get_hai34_array(self.tehai)
        tehai_str = ''
        for i in range(34):
            tehai_str += str(te[i]) + ','
        args.append(tehai_str)
        fuuro_str = ''
        ankan_num = 0
        aka_num = 0
        for i in range(4):
            if len(self.fuuro) > i:
                if self.fuuro[i].fuuro_type == Fuuro_Type.FT_ANKAN:
                    ankan_num += 1
                if self.fuuro[i].hai % 10 == 0 or self.fuuro[i].consumed[0] % 10 == 0 or self.fuuro[i].consumed[1] % 10 == 0:
                    aka_num += 1
                if self.fuuro[i].fuuro_type == Fuuro_Type.FT_CHI:
                    min_hai = min(get_hai34(self.fuuro[i].hai), get_hai34(self.fuuro[i].consumed[0]))
                    fuuro_str += str((min_hai // 9 * 7) + (min_hai % 9)) + ','
                else:
                    fuuro_str += str(get_hai34(self.fuuro[i].hai) + 21) + ','
            else:
                fuuro_str += '-1,'
        args.append(fuuro_str)
        args.append(str(len(self.fuuro)))
        dora_str = ''
        for i in range(5):
            if len(self.dora_marker) > i:
                dora_str += str(dora_marker_to_dora(self.dora_marker[i])) + ','
            else:
                dora_str += '-1,'
        args.append(dora_str)
        args.append(str(self.jikaze))
        args.append(str(self.bakaze))
        args.append(str(ankan_num))
        aka_num += self.tehai[10] + self.tehai[20] + self.tehai[30]
        args.append(str(aka_num))
        if self.jikaze == 0:
            args.append('1')
        else:
            args.append('0')
        args.append(str(self.honba))
        args.append(str(self.kyotaku))
        nokori_str = ''
        for i in range(34):
            nokori_str += str(self.nokori_array[i]) + ','
        args.append(nokori_str)
        #print(args)
        assert len(args) == 13, "The number of args is not 13"
        try:
            proc = subprocess.run(args, stdout=subprocess.PIPE)
        except:
            assert False, "look_ahead_feature_error"
        ret = np.zeros((2*SCORE_NUM*(SHANTEN_LIMIT+1),34), dtype=np.int)
        tmp = proc.stdout.decode("utf8")
        #sum = 0
    ...
    def to_numpy_fuuro_look_ahead_feature(self, target_hai, consumed_hai, fuuro_kind):
        args = ['./a.out']
        te = get_hai34_array(self.tehai)
        if fuuro_kind == Fuuro_Type.FT_ANKAN:
            t = -1
            c = [hai_str_to_int(consumed_hai[0]), hai_str_to_int(consumed_hai[1])]
        else:
            t = hai_str_to_int(target_hai)
            c = [hai_str_to_int(consumed_hai[0]), hai_str_to_int(consumed_hai[1])]
        nokori_copy = copy.deepcopy(self.nokori_array)
        if fuuro_kind >= 3:#kan
            te[get_hai34(c[0])] = 0
            nokori_copy[get_hai34(c[0])] = 0
        else:
            te[get_hai34(c[0])] -= 1
            te[get_hai34(c[1])] -= 1
        tehai_str = ''
        ...
        for i in range(4):
            if i < len(self.fuuro):
                ...
                if self.fuuro[i].fuuro_type == Fuuro_Type.FT_CHI:
                    ...
                    fuuro_str += str(get_hai34(self.fuuro[i].hai) + 21) + ','
            elif i == len(self.fuuro) and fuuro_kind != Fuuro_Type.FT_KAKAN:
                fuuro_num += 1
                if fuuro_kind == Fuuro_Type.FT_ANKAN:
                    ankan_num += 1
                if fuuro_kind == Fuuro_Type.FT_CHI:
                    min_hai = min(get_hai34(t), get_hai34(c[0]))
                    fuuro_str += str((min_hai // 9 * 7) + (min_hai % 9)) + ','
                else:
                    fuuro_str += str(get_hai34(c[0]) + 21) + ','
            else:
                fuuro_str += '-1,'
        args.append(fuuro_str)
        args.append(str(fuuro_num))
        dora_str = ''
        for i in range(5):
            if len(self.dora_marker) > i:
                dora_str += str(dora_marker_to_dora(self.dora_marker[i])) + ','
    ...
    def to_numpy(self, tehai_flag):
        ...
        if tehai_flag:
            return np.concatenate([self.to_numpy_tehai(), self.to_numpy_look_ahead_feature(), visible])
        else:
            return visible

    def to_numpy_fuuro2(self, tehai_flag, target_hai, consumed_hai, fuuro_kind):
        visible = np.concatenate([self.to_numpy_fuuro(), self.to_numpy_jikaze(), self.to_numpy_kawa(), self.to_numpy_score()])
        if tehai_flag:
            return np.concatenate([self.to_numpy_tehai(), self.to_numpy_look_ahead_feature(), self.to_numpy_fuuro_look_ahead_feature(target_hai, consumed_hai, fuuro_kind), visible])
        else:
            return visible

class Game_State:
    def __init__(self, bakaze, kyoku, honba, kyotaku, scores, oya, dora_marker, tehai_array, nokori_array):
        self.bakaze = bakaze
        self.kyoku = kyoku
        self.honba = honba
        self.kyotaku = kyotaku
        ...
        if is_valid_hai(dora_marker):
            self.dora_marker.append(dora_marker)
        self.player_state = [Player_State(scores[i], (i - oya + 4) % 4, bakaze, tehai_array[i], nokori_array, honba, kyotaku, dora_marker) for i in range (4)]
    ...
    def go_next_state(self, action_json):
        ...
        if action_json["type"] == "dora":
            d = hai_str_to_int(action_json["dora_marker"])
            self.dora_marker.append(d)
            self.nokori_array[get_hai34(d)] -= 1
            ...
            self.player_state[actor].tehai[hai] -= 1
            self.nokori_array[get_hai34(hai)] -= 1
            ...
            self.nokori_array[get_hai34(hai)] -= 1
            self.nokori_array[get_hai34(consumed[0])] -= 1
            self.nokori_array[get_hai34(consumed[1])] -= 1
            ...
            assert self.player_state[target].kawa[-1].hai == hai, "pon_hai_error"
            self.player_state[target].kawa[-1].is_taken = True
            self.nokori_array[get_hai34(hai)] -= 3
            ...
            assert self.player_state[target].kawa[-1].hai == hai, "pon_hai_error"
            self.player_state[target].kawa[-1].is_taken = True
            self.nokori_array[get_hai34(hai)] -= 4
    ...
    def to_numpy_fuuro2(self, my_pid, target_hai, consumed_hai, fuuro_kind):
        # my_pid is 0,1,2,3
        ps = np.concatenate([self.player_state[(my_pid + i)%4].to_numpy_fuuro2(i == 0, target_hai, consumed_hai, fuuro_kind) for i in range(4)])
        ps2 = np.concatenate([ps, self.to_numpy_bakaze(), self.to_numpy_kyoku(), self.to_numpy_honba(), self.to_numpy_kyotaku(), self.to_numpy_dora()])
        return ps2
...
def get_game_state_start_kyoku(action_json_dict):
    ...
    nokori_array = [4 for i in range(34)]
    nokori_array[get_hai34(hai_str_to_int(action_json_dict["dora_marker"]))] -= 1;

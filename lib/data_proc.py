            ...
            if action["type"] == "tsumo" or action["type"] == "dahai":
                for legal_action in legal_actions_all[i]:
                    if legal_action["type"] == "chi":
                        ...
                        x = game_state.to_numpy_fuuro2(legal_action["actor"], legal_action["pai"], legal_action["consumed"], 1)
                        self.x_chi.append(x)
                    ...
                    if legal_action["type"] == "pon":
                        ...
                        #x = game_state.to_numpy(legal_action["actor"])
                        #x = game_state.to_numpy_fuuro(legal_action["actor"], legal_action["pai"], legal_action["consumed"])
                        x = game_state.to_numpy_fuuro2(legal_action["actor"], legal_action["pai"], legal_action["consumed"], 2)
                        self.x_pon.append(x)
                    ...
                    if  legal_action["type"] == "kakan" or legal_action["type"] == "ankan" or legal_action["type"] == "daiminkan":
                        ...
                        if fuuro_kind == 4:
                            x = game_state.to_numpy_fuuro2(legal_action["actor"], "ankan", legal_action["consumed"], fuuro_kind)
                        else:
                            x = game_state.to_numpy_fuuro2(legal_action["actor"], legal_action["pai"], legal_action["consumed"], fuuro_kind)
                        self.x_kan.append(x)
...
def get_current_feature(current_record, legal_actions):
    ...
    for legal_action in legal_actions:
        if legal_action["type"] in ["chi"]:
            x = game_state.to_numpy_fuuro2(legal_action["actor"], legal_action["pai"], legal_action["consumed"], 1)
            features[legal_action["actor"]]["others"].append((legal_action, x))
        elif legal_action["type"] in ["pon"]:
            x = game_state.to_numpy_fuuro2(legal_action["actor"], legal_action["pai"], legal_action["consumed"], 2)
            features[legal_action["actor"]]["others"].append((legal_action, x))
        elif legal_action["type"] == "reach":
            features[legal_action["actor"]]["reach"] = game_state.to_numpy(legal_action["actor"])
        elif legal_action["type"] in ["daiminkan"]:
            x = game_state.to_numpy_fuuro2(legal_action["actor"], legal_action["pai"], legal_action["consumed"], 3)
            features[legal_action["actor"]]["others"].append((legal_action, x))
        elif legal_action["type"] in ["ankan"]:
            x = game_state.to_numpy_fuuro2(legal_action["actor"], "ankan", legal_action["consumed"], 4)
            features[legal_action["actor"]]["others"].append((legal_action, x))
        elif legal_action["type"] in ["kakan"]:
            x = game_state.to_numpy_fuuro2(legal_action["actor"], legal_action["pai"], legal_action["consumed"], 5)
            features[legal_action["actor"]]["others"].append((legal_action, x))
